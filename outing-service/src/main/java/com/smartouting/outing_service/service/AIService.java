...
/**
 * Enhanced AI Service — Multi-factor urgency analysis engine.
 *
 * Scoring is additive across 5 dimensions:
 *   1. Category base score  (0–50)
 *   2. Severity modifiers   (+25 max)
 *   3. Context reducers     (negative)
 *   4. Quality bonus        (+10 max)
 *   5. Risk/flag penalties  (negative)
 *
...
public class AIService {
    ...
    static {
        ...
        ACADEMIC_KEYWORDS.put("project submission", 12);
    }
    ...
    static {
        ...
        SEVERITY_AMPLIFIERS.put("cannot stand", 14);
    }

    // ── CONTEXT reducers (negative weights) ────────────────────────────────────
    ...
    public AIAnalysisResult analyzeRequest(String reason) {
        if (reason == null || reason.isBlank()) {
            return buildResult("INSUFFICIENT_INFO", 5, 10,
                    List.of("Empty or missing reason"),
        ...
        if (wordCount < 3) {
            flags.add("Description too vague (less than 3 words)");
            riskPenalty += 10;
        }
        if (category.startsWith("MEDICAL") && leis > 0) {
            flags.add("Mixed signals: medical reason with leisure context");
            riskPenalty += 8;
        }
        ...
        String explanation = "Category: " + category + " | Urgency: " + urgency
                + "/100 | Confidence: " + confidence + "%."
                + (severityBoost > 0 ? " Severity keywords boosted by +" + severityBoost + "." : "")
                + (qualityBonus   > 0 ? " Detailed description: +" + qualityBonus + "." : "")
                + (riskPenalty    > 0 ? " Risk penalty: -" + riskPenalty + "." : "")
                + (flags.isEmpty() ? "" : " Flags: " + String.join("; ", flags) + ".");

        return buildResult(category, urgency, confidence, flags, recommendation, explanation);
    }
    ...
    private AIAnalysisResult buildResult(String category, int urgency, int confidence,
                                         List<String> flags, String recommendation,
                                         String explanation) {
